...
function ScreenshotPicker({ label, file, onChange, tone }: { label: string; file: File | null; onChange: (file: File | null) => void; tone: "indigo" | "emerald" }) {
  ...
  return <div><label className="block"><FieldLabel>{label}</FieldLabel><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => onChange(event.target.files?.[0] ?? null)} className={`mt-2 w-full rounded-lg border border-white/10 bg-[#080b10] px-3 py-2.5 text-sm text-slate-200 outline-none focus:border-indigo-400 file:mr-3 file:rounded-md file:border-0 file:px-3 file:py-1.5 file:text-xs file:font-medium ${toneClass}`} /></label>{previewUrl ? <figure className="mt-4 overflow-hidden rounded-xl border border-white/10 bg-[#080b10]"><img src={previewUrl} alt={`${label} preview`} className="max-h-[520px] w-full object-contain" /><figcaption className="flex flex-wrap items-center justify-between gap-2 border-t border-white/[0.07] px-3 py-2 text-xs text-slate-500"><span className="truncate">{file?.name}</span><span>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : ""}</span></figcaption></figure> : <div className="mt-4 grid min-h-40 place-items-center rounded-xl border border-dashed border-white/10 bg-[#080b10] px-4 text-center text-xs text-slate-600">Your chart screenshot preview will appear here.</div>}</div>;
}
